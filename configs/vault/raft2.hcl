...
storage "raft" {
  ...
  retry_join {
    ...
    leader_client_cert_file = "/vault/certs/vault-2.crt"
    leader_client_key_file = "/vault/certs/vault-2.key"
  }
  retry_join {
...
listener "tcp" {
  ...
  tls_cert_file   = "/vault/certs/vault-2.crt"
  tls_key_file    = "/vault/certs/vault-2.key"
