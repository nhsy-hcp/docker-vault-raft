...
storage "raft" {
  ...
  retry_join {
    ...
    leader_client_cert_file = "/vault/certs/vault-3.crt"
    leader_client_key_file = "/vault/certs/vault-3.key"
  }
  retry_join {
...
listener "tcp" {
  ...
  tls_cert_file   = "/vault/certs/vault-3.crt"
  tls_key_file    = "/vault/certs/vault-3.key"
